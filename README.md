...
```js
db.users.updateOne({ name: "Hexus" }, { $set: { name: "Texas", id: 104 } })
db.users.updateOne({ name: "lexus" }, { $set: { contact: 777777 } })
```

**Nested field update** using dot notation:
```js
db.users.updateOne({ name: "lexus" }, { $set: { "address.city": "LA" } })
```

**`$unset`** — Remove a field from a document:
```js
db.users.updateOne({ name: "lexus" }, { $unset: { "address.city": "" } })
```
...
```js
db.users.deleteOne({ id: 103 })        // deletes first matching document
...
|----------|-------------|
| `$push` | Appends a single element to an array |
| `$pull` | Removes all occurrences of a specific value |
| `$pop` | Removes the first (`-1`) or last (`+1`) element |
| `$addToSet` | Adds an element only if it doesn't already exist |
| `$each` | Used with `$push` or `$addToSet` to insert multiple values at once |
...
| App | Port | Route Prefix | Handles |
|-----|------|--------------|---------|
| `app` | 3030 | `/user-api` | User CRUD + Login |
| `pro` | 4040 | `/product-api` | Product CRUD |
...
| `price` | Number | Required, min: 10,000, max: 50,000 |
...
| GET | `/products/:id` | Get a product by MongoDB ObjectId |
| PUT | `/products/:id` | Update a product (`$set` + `runValidators`) |
